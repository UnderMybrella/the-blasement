...
interface BlaseballDatabaseShopSetupEndpoint : BlaseballEndpoint {
    ...
    object Live : BlaseballDatabaseShopSetupEndpoint {
        override suspend fun getDataFor(league: BlasementLeague, request: Request): JsonElement? =
            league.httpClient.get("https://www.blaseball.com/database/shopSetup")

        override fun describe(): JsonElement? =
            JsonPrimitive("live")
    }

    data class Static(val data: JsonElement?): BlaseballDatabaseShopSetupEndpoint {
    ...
    companion object {
        infix fun loadFrom(config: JsonElement?): KorneaResult<BlaseballDatabaseShopSetupEndpoint?> {
